...
const FormGeneral: React.FC<FormComponent> = ({
  id,
  getData,
  setIsLoading,
}) => {
  ...
  const handleGetDataDetail = async () => {
    try {
      const response = await axios.get(
        `/api/admin/master_data/classification/${id}?type=postGeneralClassification`
      );
      setFormData({
    ...
    try {
      if (mode === "PUT") {
        const response = await axios.put(
          `/api/admin/master_data/classification/${id}?type=updateGeneralClassification`,
          formData
        );
        ...
        const response = await axios.post(
          "/api/admin/master_data/classification?type=postGeneralClassification",
          formData
        );
  ...
  return (
    <>
      <Modal
        ...
        title={
          mode === "PUT"
            ? "Edit Generation Classification"
            : "Create New Generation Classification"
        }
      >
        <div>
          ...
          >
            <Input
              placeholder="code ex. Baby Boomers"
              name="label"
              onChange={handleInputChange}
