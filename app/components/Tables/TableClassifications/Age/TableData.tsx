...
const TableDataAge = () => {
  ...
  const table = useMantineReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      >
        <FormAge getData={getData} setIsLoading={setIsLoading} />
    ...
    renderRowActions: ({ row }) => (
      <>
        <Box className="flex flex-nowrap gap-2">
          <FormAge id={row.id} getData={getData} setIsLoading={setIsLoading} />
          <ButtonDelete
            id={row.id}
            url="Classification"
            getData={getData}
            setIsLoading={setIsLoading}
            params="type=deleteAgeClassification"
          />
        </Box>
      </>
    ),
