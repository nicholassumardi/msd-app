...
const FormCategory: React.FC<FormComponent> = ({
  id,
  getData,
  setIsLoading,
}) => {
  ...
  const handleGetDataDetail = async () => {
    try {
      const response = await axios.get(
        `/api/admin/master_data/job_family/category/${id}`
      );
    ...
    try {
      if (mode === "PUT") {
        ...
        const response = await axios.post(
          "/api/admin/master_data/job_family/category?type=store",
          formData
        );
        if (response.status === 201) {
          SuccessNotification({
            title: "Success",
            message: "category data successfully created",
          });
          close();
        }
        setIsLoading(true);
  ...
  return (
    <>
      <Modal
        ...
        title={
          <Text fw={700} size="xl">
            {mode === "PUT" ? "Edit Category" : "Create New Category"}
          </Text>
        }
      >
        <div>
          <Input.Wrapper withAsterisk label="Name" error={error?.name ?? ""}>
            <Input
