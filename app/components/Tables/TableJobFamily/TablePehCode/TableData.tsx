...
import FormPehCode from "./Form";
import ButtonExport from "@/components/common/ButtonExport";

const columns: MRT_ColumnDef<PehCode>[] = [
  {
...
const TableDataPehCode = () => {
  const [data, setData] = useState<PehCode[]>([]);
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        "/api/admin/master_data/job_family/peh_code?type=showPagination",
        {
          params: {
  ...
  const handleExportRows = (rows: MRT_Row<PehCode>[]) => {
  ...
  const table = useMantineReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      ...
      >
        <FormPehCode getData={getData} setIsLoading={setIsLoading} />
...
export default TableDataPehCode;
